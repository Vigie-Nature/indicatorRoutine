estimateTrends <- function(
  ...
  #######################
  if(makeGammTrend){
    cat(paste("Chemin est", here::here("outputs", repo, "models", "longTermTrend", paste0(sp, ".rdata"))))
    gammVariations <- makeGAM2(data = dataSp, interestVar = interestVar, fixedEffects = fixedEffects,
                              factorVariables = factorVariables, randomEffects = randomEffects,
